...
import com.example.contract.RedeemTokenState
 ...
 */
object RedeemWorkFlow {
    class Initiator(val tokenState: RedeemTokenState,
                    val otherParty: Party): FlowLogic<RedeemFlowResult>() {
        /**
         * The progress tracker checkpoints each stage of the flow and outputs the specified messages when each
         * checkpoint is reached in the code. See the 'progressTracker.currentStep' expressions within the call() function.
         */
        companion object {
            object CONSTRUCTING_OFFER : ProgressTracker.Step("Constructing token order for Redeeming rewards")
            object SENDING_OFFER : ProgressTracker.Step("Sending token order to vendor for review.")
            object RECEIVED_PARTIAL_TRANSACTION : ProgressTracker.Step("Received partially signed transaction from vendor.")
            ...
            object SENDING_FINAL_TRANSACTION : ProgressTracker.Step("Sending fully signed transaction to vendor.")
         ...
         */
        @Suspendable
        override fun call(): RedeemFlowResult {
            ...
            try {
                ...
                send(otherParty, vtx)
                return RedeemFlowResult.Success("Transaction id ${vtx.id} committed to ledger.")
            } catch(ex: Exception) {
                // Just catch all exception types.
                return RedeemFlowResult.Failure(ex.message)
            }
        }
    }

    class Acceptor(val otherParty: Party): FlowLogic<RedeemFlowResult>() {
        companion object {
        ...
        override fun call(): RedeemFlowResult {
            try {
                ...
                return RedeemFlowResult.Success("Transaction id ${ntx.id} committed to ledger.")
            } catch (ex: Exception) {
                return RedeemFlowResult.Failure(ex.message)
            }
        }
 ...
 */
sealed class RedeemFlowResult {
    class Success(val message: String?): RedeemFlowResult() {
        override fun toString(): String = "Success($message)"
    }

    class Failure(val message: String?): RedeemFlowResult() {
